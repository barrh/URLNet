...
class URLNet(nn.Module):
    def __init__(self, char_ngram_vocab_size, word_ngram_vocab_size, char_vocab_size,
        ...
        # l2_loss = tf.constant(0.0) // need to understand why is that
        # Embedding
        if mode == 4 or mode == 5:
            self.char_emb_w = torch.FloatTensor(char_ngram_vocab_size, embedding_size).uniform_(-1.0, 1.0)
        if mode == 2 or mode == 3 or mode == 4 or mode == 5:
            self.word_emb_w = torch.FloatTensor(word_ngram_vocab_size, embedding_size).uniform_(-1.0, 1.0)
        if mode == 1 or mode == 3 or mode == 5:
            self.char_seq_emb_w = torch.FloatTensor(char_vocab_size, embedding_size).uniform_(-1.0, 1.0)
        if mode == 4 or mode == 5:
