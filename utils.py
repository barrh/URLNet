...
def create_tokenizer_from_alexa(max_length_words, urls):
    tokenizer = tf_utils.Tokenizer(oov_token="<UNK>", split='.', filters='', lower=True)
    complete_urls = []
    for word in urls:
...
def get_word_vocab(urls, max_length_words, min_word_freq=0):
    ...
    vocab_processed = tf_utils.pad_sequences(x, maxlen=max_length_words, padding='post',
                                                                    truncating='post')
...
def urls_to_ngrams(tokenizer, train_urls, high_freq_words, max_len_words, delimit_mode, max_len_subwords):
    ...
    train_sequences_padded = tf_utils.pad_sequences(train_sequences,
