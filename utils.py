...
class N_grams():
    ...
    def __init__(self, len_ngrams=0, path=None,tld_name="tld",domain_name="domain_names",pathes_name="pathes"):
        ...
        if path:
            ...
            self.tlds = {tld: k for k, tld in enumerate(ngrams_extracted[tld_name])}
            self.domain = {domain_ngrams: k + len(self.tlds) for k, domain_ngrams in
                           enumerate(ngrams_extracted[domain_name])}
            self.path = {path_ngrams: k + len(self.tlds) + len(self.domain) for k, path_ngrams in
                         enumerate(ngrams_extracted[pathes_name])}


    def __bool__(self):
        return self.len_ngrams > 0
    ...
    def get_bow_domain(self, clean_url):
        return {self.domain[ngram] for ngram in ngrams(clean_url,3,self.len_ngrams+1).intersection(self.domain)}

    def get_bow_path(self, clean_url):
        return {self.path[ngram] for ngram in ngrams(clean_url,3,self.len_ngrams+1).intersection(self.path)}

    def get_ngram_domain(self, clean_url):
        return {ngram for ngram in ngrams(clean_url,3,self.len_ngrams+1).intersection(self.domain)}

    def get_ngram_path(self, clean_url):
        return {ngram for ngram in ngrams(clean_url,3,self.len_ngrams+1).intersection(self.path)}
    ...
    def get_clean_url(url):
        try:
            ...
            return None, None
    def texts_to_sequences(self,url_list,labels):
        url_final_list = []
        labels_final_list = []
        for url,label in zip(url_list,labels):
            ...
            if tld:
                ...
                labels_final_list.append(label)
                ...
                if tld in self.tlds:
                    ind_of_ngrams = {self.tlds[tld]}
                path_to_file = url.replace(clean_url + ".{}".format(tld), "")
                domain_relevant_words = self.get_ngram_domain(clean_url)
                while(len(domain_relevant_words)):
                    res = max(domain_relevant_words, key=len)
                    clean_url = clean_url.replace(res,"<{}")
                ind_of_ngrams |= (self.get_bow_domain(clean_url))
                if path_to_file:
                    ind_of_ngrams |= self.get_bow_path(path_to_file)
